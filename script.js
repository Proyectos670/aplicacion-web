...
const formatStates = { '01': {},  '16': {},  '17': {} ,  '18': {} ,  '19': {} ,  '24': {},  '25': {}    };
const tableHeaders = {
  ...
  '17': ['Fecha','Hora',
        'Turbiedad (NTU) Pozo Vy-64', 'pH (Unidades de pH) Pozo Vy-64', 'Color (UPC) Pozo Vy-64', 'Fosfatos (mg/PO4) Pozo Vy-64', 
        'Turbiedad (NTU) Tanque-20M3', 'pH (Unidades de pH) Tanque-20M3', 'Color (UPC) Tanque-20M3', 'Fosfatos (mg/PO4) Tanque-20M3', 
        'Turbiedad (NTU) Tanque-40M3', 'VMP (2 NTU) Tanque-40M3', 'pH (Unidades de pH) Tanque-40M3', 'VMP (6.5/9) Tanque-40M3', 
        'Color (UPC) Tanque-40M3', 'VMP (15 UPC) Tanque-40M3', 'Fosfatos (mg/PO4) Tanque-40M3', 'VMP (0.5 mg/l) Tanque-40M3', 
...
function loadData() {
  ...
  if (storedFormats) {
    Object.keys(storedFormats).forEach(formatCode => {
      // Inicializa los datos correctamente si no existen
      if (!storedFormats[formatCode].data) {
        storedFormats[formatCode].data = [];  // Inicializa data como un array vacío
      }
      if (storedFormats[formatCode].rowCount === undefined) {
        storedFormats[formatCode].rowCount = 0;  // Inicializa el contador de filas
      }
      if (storedFormats[formatCode].columnCount === undefined) {
        storedFormats[formatCode].columnCount = 0;  // Inicializa el contador de columnas
      }
    });
    Object.assign(formatStates, storedFormats);
  }
  if (storedHeaders) {
    Object.assign(tableHeaders, storedHeaders);
  }
  loadFormatButtons();  // Recargar los botones de formato
}
...
function addRow() {
  ...
  // Verifica si los datos existen; si no, inicialízalos
  if (!state.data) {
    state.data = [];  // Inicializa un array vacío si no existen los datos
  }

  state.rowCount++; // Incrementa el contador de filas
  state.data.push(new Array(state.columnCount).fill('')); // Agregar una fila vacía

  console.log('Estado antes de agregar fila:', state);  // Verificar el estado antes de agregar

  saveData();  // Guardar el estado en localStorage
  createTable(); // Volver a crear la tabla con la nueva fila

  console.log('Estado después de agregar fila:', state);  // Verificar el estado después de agregar
}
